...
public abstract class SourceEditorMediator {
	...
	public SourceEditorMediator() throws ClassCastException, IllegalAccessException, InstantiationException,
    	...
    	*/
		new RemoteWindowPopupDecorator().activate(this.frame, 6099);
	...
	public SourceEditorMediator(List<URI> uriList, List<File> fileList) throws ClassCastException, IllegalAccessException,
		InstantiationException, ClassNotFoundException, UnsupportedLookAndFeelException {
		this();
		for(URI uri: uriList){
			this.menuBarMediator.open(uri, false, null);
			this.menuBarMediator.updateMenu();
		}
		for(File file: fileList){
			this.menuBarMediator.open(file);
			this.menuBarMediator.updateMenu();
		}
	}
