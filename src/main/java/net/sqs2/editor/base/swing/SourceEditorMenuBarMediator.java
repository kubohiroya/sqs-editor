...
public abstract class SourceEditorMenuBarMediator {
	...
	public DOMTreeSource createSource(URI uri, boolean readonly, String title) throws SourceException {
		return (DOMTreeSource) mediator.getSourceManager().createSource(uri, readonly, title);
	}

	public void open(URI uri, boolean readonly, String title) {
		try {
			mediator.getFrame().setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
			DOMTreeSource source = createSource(uri, readonly, title);
